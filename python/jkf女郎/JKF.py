
import requests
import re
import json
import os
import requests
import urllib.request
import time
import base64
import datetime
import sys
###

import CustomEncryption

# 引入 Beautiful Soup 模组
from bs4 import BeautifulSoup


###
CustomEncryption = CustomEncryption.CustomEncryption()


def inint():

    temple_ = {
        # 搜寻玛
        "serchcode":  '',
        # 网址
        "host": "",
        # 作者
        "auth": '',
        # 人员
        "personnel": '',
        # 标题
        "title": '',
        # 关键字
        "series": '',
        # 图片
        "img":  [],
        'img_rows': 0,
        ##########################
        # 关键字
        "keywords": "",
        # 简介
        "description": "",
        # 部分
        "section": []


    }

    return temple_


def cr_dir(FILE_PATH):

    if os.path.isdir(FILE_PATH):  # 不用加引号，如果是多级目录，只判断最后一级目录是否存在
        pass
    else:

        # 只能创建单级目录，用这个命令创建级联的会报OSError错误         print 'mkdir ok
        os.mkdir(FILE_PATH)

    return True


def get_soup(url):

    headers = {
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36'}
    headers['Referer'] = url
    r = requests.get(url, headers=headers)

    html_doc = r.text
    # print(html_doc)
    # .decode('utf-8', 'ignore')
    # 以 Beautiful Soup 解析 HTML 程式码
    soup = BeautifulSoup(html_doc, 'html.parser')
    return soup


def singe_page(url):

    ###
    # title
    # <meta property="article:section" content="正妹">
    # <meta property="article:section2" content="台湾正妹">
    # <meta name="keywords" itemprop="keywords" content="阿粗严选,阿粗学长,jieeeshin,洁 心,正妹,奶好大" />
    # <meta name="description" content="这次受到阿粗学长的感召！要推荐大家「阿粗」分享ㄉ正妹～以前都觉得全糖妹才是真妹子，现在阅妹无数之后才发现臭脸妹才是王道啊！！有凶凶的脾气，才有凶凶的身材 ... ♥阿粗严选♥臭脸妹身材好暴力！海岛晒乳「绝美水滴胸型」好Q好饱满...老司机嗨到起秋~  (阿粗严选,阿粗学长,jieeeshin,洁 心,正妹,奶好大)"/>

    ##
    temple_ = inint()

    file_s = []
    ###

    soup = get_soup(url)
    # print(soup)

    div_ = soup.find_all('ignore_js_op')
    # print(div_)
    for target_list in div_:
        file_ = target_list.find('img').get('file')
        # print(file_)
        if file_ == None:
            pass
        else:
            file_s.append(file_)
    # 图片
    if file_s == []:
        # pcb
        img = soup.find_all(
            'img', {'onclick': "zoom(this, this.src, 0, 0, 0)"})
        for target_list in img:
            file_ = target_list.get('src')
            # print(file_)
            if file_ == None:
                pass
            else:
                file_s.append(file_)

    temple_['img'] = file_s
    temple_['img_rows'] = len(file_s)

    if temple_['img_rows'] == 0:
        print('圖片為空白')
        return True

    # return True
    ##
    temple_['host'] = url
    temple_['title'] = soup.find("meta", property="og:title")['content']

    ####
    temple_['section'].append('JKF女郎')
    # try:
    #     temple_['section'].append(
    #         soup.find("meta",  property="article:section")['content'])
    #     temple_['section'].append(
    #         soup.find("meta",  property="article:section2")['content'])
    # except:
    #     pass

    # print(temple_)
    # <meta property="dable:item_id" content="4952696">
    # temple_['serchcode'] = soup.find("meta", property="dable:item_id")['content']
    # 使用网址 进行 b16encode
    # print(temple_)
    # return True
    temple_['serchcode'] = CustomEncryption.b16_encode(url)

    temple_['keywords'] = soup.find(attrs={"name": "keywords"})['content']
    temple_['description'] = soup.find(
        attrs={"name": "description"})['content']
    # print(temple_)
    # return True
    # IMG path
    temple_['img'] = get_to_image(temple_['serchcode'],  temple_[
                                  'img'],  temple_['img_rows'])

    # JSON path
    get_to_json(temple_,  temple_['serchcode'])

    return True


def get_to_json(content, _prex):

    ###
    path = './json/'
    cr_dir(path)

    ###
    filename = path + str(_prex) + '.json'
    # print(filename)
    # print(_prex)
    # print(content)
    # Writing JSON data
    with open(filename, 'w') as f:
        json.dump(content, f)

    return True


def get_to_image(_prex,  img,  count):
        ###
    path = './images/'
    cr_dir(path)
    path = path + _prex + '/'
    cr_dir(path)
    #################################
    true_local_path = []
    #################################

    row = 0
    for url_img in img:
        ##
        row += 1
        html = requests.get(url_img)
        img_name = path + str(row) + '.png'
        ####
        true_local_path.append(img_name)
        # pass
        with open(img_name, 'wb') as file:  # 以byte的形式将图片数据写入
            file.write(html.content)
            file.flush()
        file.close()  # close file
        print('第 %d 张' % (row))

        time.sleep(0.1)

    # print(true_local_path)
    return true_local_path


def list_page(url):
    soup = get_soup(url)

    # <div class="l_sPic"><
    div = soup.find_all('a', {'onclick': "atarget(this)"})
    # print(div)
    for _a in div:
        url = _a.get('href')
        title = _a.get('title')
        print(url, ' : ', title)
        url = 'https://www.jkforum.net/' + url
        singe_page(url)

    # table = soup.find('table', {'id': "threadlisttableid"})
    # # print(table)
    # for tbody in table:
    #     tbody_ = tbody.find('tbody')
    #     print(tbody)

    return True


# 单一面面
# url = 'https://www.jkforum.net/thread-10605198-1-1.html'
# url = "https://www.jkforum.net/thread-10334056-1-1.html"
# singe_page(url)

# LIST  首页›JKF›JKF女郎
# url = 'https://www.jkforum.net/forum-1112-1.html'
# list_page(url)


a = sys.argv
b = len(sys.argv)
# print(a)
# print(b)
if b != 1:
    _urk = "https://www.jkforum.net/forum-1112-{0}.html"
    # 网址
    # print(a[1])
    rrr = _urk.format(a[1])
    # list_page(rrr)

    print(rrr)
    list_page(rrr)
    # while True:
    #     time.sleep(10)
    #     print('XXX')
